...
export default function TaskItem({ task, onUpdateStatus, onDelete }: TaskItemProps) {
  ...
  return (
    <div className={`flex items-center justify-between p-3 rounded-lg border border-gray-700 bg-gray-800/50 ${task.status === "done" ? "opacity-50" : ""}`}>
      ...
      <div className="flex items-center gap-2 ml-3 shrink-0">
        <button
          ...
          className={`px-3 py-1 rounded text-sm font-medium cursor-pointer ${action.classes}`}
        >
          {action.label}
        </button>
        <button
          onClick={() => onDelete(task.id)}
          className="px-2 py-1 rounded text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 cursor-pointer"
        >
          ✕
        </button>
      </div>
